...
// CODE HERE
function changeSayHelloButton(){
    sayHelloButton.style.backgroundColor='black'
    sayHelloButton.style.color='white'

}
sayHelloButton.addEventListener('mouseover',changeSayHelloButton)
// PROBLEM 3
/*
...
sayHelloButton.addEventListener('mouseout',changeSayHelloButton)
// PROBLEM 4
/*
...
const ohMy = () => {
    ...
    .then(res=>{
        ...
        for(let i=0; i<res.data.length; i++){
            let createNewPelement=document.createElement('p')
            createNewPelement.textContent=res.data[i]
            body.appendChild(createNewPelement)
          }
    })
    .catch((error)=>
    console.log(error +" ohmy")
    )
}
...
*/
//const requestURL='http://localhost:3000/repeat/'
const repeatMyParam = () => {
    ...
    .catch((error)=>
    console.log(error+" repeatmyparam")
    )
...
// CODE HERE
let getBtn=document.querySelector('#query-button')
function getRequestQuery(){
    axios.get('http://localhost:3000/query-test?greeting=hello')
    .then(function(res){
...
getBtn.addEventListener('click', getRequestQuery)
...
addFood.addEventListener('click',createFood)
